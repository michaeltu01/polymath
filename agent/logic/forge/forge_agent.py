...
"""
            ...
            with open(
                ...
                file.flush() # ensure that all data is written to disk

                print("Solver input file:", solver_input_file)
                try:
                    exit_code, stdout, stderr = await self.util_communicate_to_sterling_client(solver_input_file)
                except TimeoutError:
                    self.__logger.exception(
                        f"""Solver timeout.
Python Code:
...
"""
            ...
            # TODO: Parse solver output to determine solver outcome
            # - If the solver produces an instance, return it as the output
            # - If the solver fails with UNSAT, retry with the retry prompt
            # - If the solver errors in any other way, return fatal error

            solver_outcome, output = self.__engine_strategy.parse_solver_output(exit_code, stdout, stderr)
    ...
    async def util_communicate_to_sterling_client(self, solver_input_file: str) -> tuple[int, str, str]:
        """
        ...
        Returns:
            exit_code: Exit code of the Sterling client process
        ...
        try:
            ...
            try: 
                ...
                exit_code, stdout, stderr = await client.run_sterling_client()
